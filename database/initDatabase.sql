...
USE uwc;
    ...
    CREATE FUNCTION isAccountExit(inputId INT)
    RETURNS BOOLEAN
    DETERMINISTIC
    BEGIN
        ...
        SELECT EXISTS(SELECT id FROM account WHERE id = inputId) INTO isExist;
...
CREATE PROCEDURE getTaskByWorker(IN userId INT)
BEGIN
    ...
    SELECT T.id,
           ...
           WO.name AS doneBy
    FROM task T
    INNER JOIN account BO ON T.backofficerId = BO.id
    INNER JOIN account WO ON T.workerId = WO.id
    WHERE T.workerId = userId;
    COMMIT;
END$$
...
CREATE PROCEDURE insertTask(IN workingTime DATE, IN status VARCHAR(10), IN backofficerId INT, IN workerId INT, IN routeId INT)
BEGIN
    IF (workingTime IS NULL OR workingTime = '') THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'working time is empty';
    END IF;
    IF (status NOT IN ('WAITING', 'DOING', 'DONE')) THEN
    ...
    IF NOT EXISTS(SELECT id FROM backofficer WHERE id = backofficerId) THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'invalid back officer id';
    END IF;
    IF NOT EXISTS(SELECT id FROM worker WHERE id = workerId) THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'invalid worker id';
    END IF;
    IF NOT EXISTS(SELECT id FROM route WHERE id = routeId) THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'invalid route id';
    END IF;

    START TRANSACTION;
    INSERT INTO task(createTime, workingTime, status, acceptTime, doneTime, backofficerId, workerId, routeId) 
    VALUES (CURDATE(), workingTime, status, NULL, NULL, backofficerId, workerId, routeId);
    COMMIT;
END$$
...
CREATE PROCEDURE updateTaskAcceptTime(IN acceptTime VARCHAR(20), IN taskId INT)
BEGIN
...
CREATE PROCEDURE updateTaskDoneTime(IN doneTime VARCHAR(20), IN taskId INT)
BEGIN
    IF NOT EXISTS(SELECT id FROM task WHERE id = taskId) THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = _utf8 'invalid id';
    END IF;
...
CREATE PROCEDURE appendRoadToRoute(IN roadId INT, IN routeId INT)
BEGIN
